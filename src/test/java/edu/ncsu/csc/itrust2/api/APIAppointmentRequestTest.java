...
public class APIAppointmentRequestTest {
    ...
    @Autowired private AppointmentRequestService arService;
    ...
    /** Tests that deleting an appointment that doesn't exist returns the proper status. */
    @Test
    @WithMockUser(
            username = "hcp",
            roles = {"HCP"})
    ...
    public void testCreateBadAppointmentRequest() throws Exception {
        ...
        mvc.perform(
                        post("/api/v1/appointmentrequests")
    ...
    public void testAppointmentRequestAPI() throws Exception {
        ...
        mvc.perform(get("/api/v1/appointmentrequest"))
        ...
        mvc.perform(get("/api/v1/appointmentrequests/" + id))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE));
        ...
        mvc.perform(
                        put("/api/v1/appointmentrequests/-1")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(TestUtils.asJsonString(appointmentForm)))
