...
public class APIPersonnelTest {
    ...
    public void testPersonnelAPI() throws Exception {
        ...
        mvc.perform(get("/api/v1/personnel/hcp"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE));

        // Edit with wrong url ID should fail
        mvc.perform(
                        put("/api/v1/personnel/badhcp")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(TestUtils.asJsonString(personnel)))
    ...
    public void testGetByRole() throws Exception {
        ...
        // Invalid get request
        mvc.perform(get("/api/v1/personnel/getbyroles/ROLE_SCHMOO"))
                .andExpect(status().is4xxClientError());
    }
    ...
    public void testRoleFiltering() throws Exception {
        ...
        final MvcResult result =
                mvc.perform(get("/api/v1/personnel/getbyroles/ROLE_HCP"))
                        .andExpect(status().isOk())
                        .andReturn();
