...
public class APIUserController extends APIController {
    ...
    /**
     * Retrieves and returns a list of all Users in the system, regardless of their classification
     * (including all Patients, all Personnel, and all users who do not have a further status
     * specified)
     *
    ...
    /**
     * Creates a new user from the RequestBody provided, validates it, and saves it to the database.
     *
     * @param userF The user to be saved
     * @return response
     */
    @PostMapping("/users")
    public ResponseEntity createUser(@RequestBody final UserForm userF) {
        if (null != userService.findByName(userF.getUsername())) {
            return new ResponseEntity(
                    errorResponse("User with the id " + userF.getUsername() + " already exists"),
                    HttpStatus.CONFLICT);
        }
        User user = null;
    ...
    /**
     * Updates the User with the id provided by overwriting it with the new User record that is
     * provided. If the ID provided does not match the ID set in the User provided, the update will
     * not take place
     *
     * @param id The ID of the User to be updated
     * @param userF The updated User to save in place of the existing one
     * @return response
     */
    @PutMapping("/users/{id}")
    ...
    /**
     * Deletes the user with the id matching the given id. Requires admin permissions.
     *
    ...
    public ResponseEntity deleteUser(@PathVariable final String id) {
        ...
        try {
            ...
            return new ResponseEntity(
                    errorResponse("Could not delete " + id + " because of " + e.getMessage()),
                    HttpStatus.BAD_REQUEST);
        }
    }
     ...
     *
     * @param role role to check for the user to have.
